...
def _ssim(img, img2):
    ...
    Returns:
        float: SSIM result.
    """
    ## wcg: the error version, the right version should be c1 = (0.01*255)**2; c2=(0.03*255)**2
    # c1 = (0.01 * 255 * 2) ** 2
    # c2 = (0.03 * 255 * 2) ** 2
    c1 = (0.01 * 255) ** 2
    c2 = (0.03 * 255) ** 2
...
def calculate_psnr_lf(
    ...
    for i in range(img.shape[0]):
        psnr.append(
            # calculate_psnr(img[i], img2[i], crop_border, input_order, test_y_channel)
            PSNR(
                img[i, crop_border:-crop_border, crop_border:-crop_border],
                img2[i, crop_border:-crop_border, crop_border:-crop_border],
                data_range=255,
            )
            if crop_border
            else PSNR(
...
def calculate_ssim_lf(
    ...
    img2=img2[:,0]
    for i in range(img.shape[0]):
        # print(img[i, crop_border:-crop_border, crop_border:-crop_border].shape)
        ssim.append(
            # calculate_ssim(
            #     img[i], img2[i], crop_border, input_order, test_y_channel, **kwargs
            # )
            SSIM(
                img[i, crop_border:-crop_border, crop_border:-crop_border],
                img2[i, crop_border:-crop_border, crop_border:-crop_border],
